...
export default function PieChart() {
  ...
  return (
    <div className="pie-chart">
      <div className="pie-chart__chart-wrap">
        <svg className="pie-chart__svg" width={260} height={260} viewBox="0 0 260 260">
          {segments.map((seg) => (
            <path
              key={seg.id}
        ...
        <div className="pie-chart__center">
          ...
          <span className="pie-chart__limit">of ${limit} limit</span>
        </div>
      </div>

      <h2 className="pie-chart__title">Spending Summary</h2>

      <div className="pie-chart__legend">
        {data.map((item) => (
          <div key={item.id} className="pie-chart__row">
            <div className="pie-chart__label-wrap">
              <div className="pie-chart__swatch" style={{ backgroundColor: item.color }} />
              <span>{item.label}</span>
            </div>
            <span className="pie-chart__amount">
              ${item.value.toFixed(2)}{' '}
              <span className="pie-chart__amount-sub">of ${item.limit.toFixed(2)}</span>
            </span>
          </div>
        ))}
      </div>
